...
function View() {
  ...
  if (symbols.length < 9 && !symbols.includes(symbol)) {
    const response = await fetch(`http://127.0.0.1:8000/api/${symbol}/`);
  ...
  useEffect(() => {
    const fetchSensexData = async () => {
      const response = await fetch('http://127.0.0.1:8000/api/BSE/');
      const data = await response.json();
      setSensexData(data);
    };
    fetchSensexData();
  ...
  useEffect(() => {
    const fetchSensexDatai = async () => {
      const response = await fetch('http://127.0.0.1:8000/api/INFY/');
      const data = await response.json();
      setSensexDatai(data);
    };
    fetchSensexDatai();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const urls = symbols.map(symbol => `http://127.0.0.1:8000/api/${symbol}/`);
